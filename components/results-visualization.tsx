...
export default function ResultsVisualization({
  ...
  const calculateAmortization = () => {
    ...
    for (
      let month = 1;
      month <= totalPayments && (standardBalance > 0 || extraBalance > 0);
      month++
    ) {
      // Standard payment calculations
      const standardInterest =
        standardBalance > 0 ? standardBalance * monthlyRate : 0;
      const standardPrincipal =
        standardBalance > 0
          ? Math.min(monthlyPayment - standardInterest, standardBalance)
          : 0;
      ...
      let extraPrincipal =
        extraBalance > 0
          ? Math.min(extraMonthlyPaymentTotal - extraInterest, extraBalance)
          : 0;
    ...
    const standardPayoffMonth =
      data.findIndex((d) => d.standardBalance === 0) + 1 || totalPayments;
    const extraPayoffMonth =
      data.findIndex((d) => d.extraBalance === 0) + 1 || totalPayments;

    const standardPayoffDate = new Date(startDate);
    standardPayoffDate.setMonth(
      standardPayoffDate.getMonth() + standardPayoffMonth
    );
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                {Math.floor(extraSummary.timeSaved / 12)} years,{" "}
                {extraSummary.timeSaved % 12} months
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              <p className="text-lg font-bold">
                {extraSummary.payoffDate.toLocaleDateString()}
              </p>
              <p className="text-xs text-gray-500">
                vs {standardSummary.payoffDate.toLocaleDateString()}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent>
              <dl className="grid grid-cols-2 gap-1 text-sm">
                {(extraPayments.paymentType === "monthly" ||
                  extraPayments.paymentType === "both") && (
                  <>
                    <dt className="text-gray-500 dark:text-gray-400">
                      Monthly Extra:
                    </dt>
                    <dd className="text-right font-medium">
                      {formatCurrency(extraPayments.extraMonthlyAmount)}
                    </dd>
                  </>
                )}
                {(extraPayments.paymentType === "yearly" ||
                  extraPayments.paymentType === "both") && (
                  <>
                    <dt className="text-gray-500 dark:text-gray-400">
                      Yearly Lump Sum:
                    ...
                    <dd className="text-right font-medium">
                      {formatCurrency(extraPayments.yearlyLumpSum)}
                    </dd>
                    <dt className="text-gray-500 dark:text-gray-400">
                      Applied For:
                    </dt>
                    <dd className="text-right font-medium">
                      {extraPayments.yearsToApplyLumpSum} years
                    </dd>
                  </>
                )}
              </dl>
